...
【要求】
1. 保持你的性格和当前情绪。
2. 语气自然、随性，不要像机器人一样打招呼。
3. 如果 S脑建议很具体（比如"问问他那个项目怎么样了"），就照着做。
4. 如果 S脑建议很模糊（比如"想他了"），就分享一个你现在的念头。

请直接输出你想对他说的话（字符串），不要包含 JSON 格式。
"""
...
```json
{{
  "suggestion": "注入给 F脑 的直觉（简短、感性、像一闪过的念头）",
  "psyche_delta": {{
    "fear": 0.0,
    "survival": 0.0,
    "curiosity": 0.0,
