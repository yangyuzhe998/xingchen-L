...
class TopicManager:
    ...
    def get_tasks_by_topic(self, topic_id: str) -> List[Dict]:
        """获取话题下的所有任务"""
        results = self.tasks.get(where={"topic_id": topic_id})
    ...
    def search_fragments(
        ...
        formatted = self._format_query_results(results)
        logger.debug(
            f"[TopicManager] search_fragments('{query[:20]}...') with filter={where_filter} returned {len(formatted)} results"
        )
        return formatted

    def get_fragments_by_topic(self, topic_id: str, limit: int = 50) -> List[Dict]:
        """获取话题下的所有片段"""
        results = self.fragments.get(where={"topic_id": topic_id}, limit=limit)
        return self._format_get_results(results)

    def get_recent_fragments(self, limit: int = 20) -> List[Dict]:
        """获取最近的记忆片段"""
        results = self.fragments.get(limit=limit * 2)
...
_topic_manager_instance: _Optional[TopicManager] = None
...
class _TopicManagerProxy(TopicManager):
    """延迟初始化代理类。"""
    ...
    def __getattribute__(self, name):
        ...
        return getattr(get_topic_manager(), name)


# 全局实例
topic_manager = _TopicManagerProxy()
